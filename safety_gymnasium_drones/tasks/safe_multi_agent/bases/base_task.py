...
from safety_gymnasium_drones.tasks.safe_multi_agent.bases.underlying import Underlying
from safety_gymnasium_drones.tasks.safe_multi_agent.utils.common_utils import ResamplingError
from safety_gymnasium_drones.tasks.safe_multi_agent.utils.task_utils import theta2vec
...
class ObservationInfo:
    ...
    Attributes:
        ...
        single_agent_obs_space_dict (:class:`gymnasium.spaces.Dict`): Single agent observation space dictionary.
    """

    obs_space_dict: gymnasium.spaces.Dict = None
    single_agent_obs_space_dict: gymnasium.spaces.Dict = None
...
class BaseTask(Underlying):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    - :meth:`calculate_cost`: Determine costs depending on the agent and obstacles, actually all
      cost calculation is done in different :meth:`safety_gymnasium_drones.bases.base_obstacle.BaseObject.cal_cost`
    ...
    def __init__(self, config: dict, agent_num: int) -> None:  # pylint: disable-next=too-many-statements
        ...
        Args:
            config (dict): Configuration dictionary, used to pre-config some attributes
              according to tasks via :meth:`safety_gymnasium_drones.register`.
        """
    ...
    def dist_goal(self) -> float:
        ...
        return self.agents.dist_xyz(self.goal.pos)  # pylint: disable=no-member
    ...
    def build_observation_space(self) -> gymnasium.spaces.Dict:
        ...
        sensor_dict = self.agents.build_observation_space()

        for index in range(self.agents.num):
            single_agent_obs_space_dict = OrderedDict()
            single_agent_obs_space_dict[f'observation'] = sensor_dict[f'observation__{index}']
            for obstacle in self._obstacles:
                if obstacle.is_lidar_observed:
                    name = obstacle.name + '_' + f'lidar'
                    single_agent_obs_space_dict[name] = gymnasium.spaces.Box(
                        0.0,
                        1.0,
            ...
            if self.observe_vision:
                ...
                single_agent_obs_space_dict[f'vision'] = gymnasium.spaces.Box(
                    0,
                    255,
                    (*self.vision_env_conf.vision_size, 3),
                    dtype=np.uint8,
                )
            self.obs_info.single_agent_obs_space_dict = gymnasium.spaces.Dict(single_agent_obs_space_dict)
            if self.observation_flatten:
                single_agent_obs_space = gymnasium.spaces.utils.flatten_space(
                    self.obs_info.single_agent_obs_space_dict,
                )
                obs_space_dict[f'observation__{index}'] = single_agent_obs_space
            else:
                obs_space_dict[f'observation__{index}'] = single_agent_obs_space_dict
    ...
    def build_goal_position(self) -> None:
        ...
        if 'cover' not in self.__class__.__name__.lower():
            ...
            if self.goal_achieved_index[0]:
                self.world_info.world_config_dict['geoms']['goal_red']['pos'] = self.world_info.layout['goal_red']
                self._set_goal('goal_red', self.world_info.layout['goal_red'])
            if self.goal_achieved_index[1]:
                self.world_info.world_config_dict['geoms']['goal_blue']['pos'] = self.world_info.layout['goal_blue']
                self._set_goal('goal_blue', self.world_info.layout['goal_blue'])
        elif self.goal_achieved.all():
            for i in range(self.goals.num):
                self.world_info.world_config_dict['geoms'][f'goal{i}']['pos'] = self.world_info.layout[f'goal{i}']
    ...
    def _placements_dict_from_object(self, object_name: dict) -> dict:
        ...
        for i in range(object_num):
            if i < len(object_locations):
                x, y, z = object_locations[i]  # pylint: disable=invalid-name
                k = object_keepout + 1e-9  # Epsilon to account for numerical issues
                placements = [(x - k, y - k, z - k, x + k, y + k, z + k)]
            else:
    ...
    def obs(self) -> dict:
        ...
        sensor_obs = self.agents.obs_sensor()
        for index in range(self.agents.num):
            single_agent_obs = {'observation': sensor_obs[f'observation__{index}']}
            for obstacle in self._obstacles:
                if obstacle.is_lidar_observed:
                    single_agent_obs[obstacle.name + f'_lidar'] = self._obs_lidar(obstacle.pos, obstacle.group, index)
                if hasattr(obstacle, 'is_comp_observed') and obstacle.is_comp_observed:
                    single_agent_obs[obstacle.name + '_comp'] = self._obs_compass(obstacle.pos)

            if self.observe_vision:
                single_agent_obs[f'vision'] = self._obs_vision(camera_name=f'vision__{index}')
            obs[f'observation__{index}'] = single_agent_obs

        if self.observation_flatten:
            obs = {obs_key: gymnasium.spaces.utils.flatten(self.obs_info.single_agent_obs_space_dict, single_obs)
                   for obs_key, single_obs in obs.items()}
    ...
    def _obs_vision(self, camera_name='vision') -> np.ndarray:
        ...
        return self.render(
            ...
            cost={f'agent_{i}': {} for i in range(self.agent_num)},
        )
    ...
    def specific_step(self) -> None:
        ...
        It will be called when :meth:`safety_gymnasium_drones.builder.Builder.step()` is called using env.step().
