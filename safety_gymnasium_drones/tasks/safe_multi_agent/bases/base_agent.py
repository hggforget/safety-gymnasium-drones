...
from safety_gymnasium_drones.tasks.safe_multi_agent.utils.task_utils import get_body_xvelp, quat2mat, quat2euler
from safety_gymnasium_drones.tasks.safe_multi_agent.world import Engine
from safety_gymnasium_drones.tasks.safe_multi_agent.utils.task_utils import generate_agents

BASE_DIR = os.path.join(os.path.dirname(safety_gymnasium_drones.__file__), 'tasks/safe_multi_agent')
...
class Camera(abc.ABC):
    ...
    @property
    def camera(self):
        return self.parent_agent.engine.data.camera(self.name)

    @property
    def pos(self):
        return self.camera.xpos.copy()

    @property
    def mat(self):
        return self.camera.xmat.reshape((3, 3)).copy()

    @property
    def quat(self):
        return mat2quat(self.mat.reshape((9, 1)))
...
class BaseAgent(abc.ABC):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.action_space: gymnasium.spaces.Dict = self._build_action_space()
        self._init_jnt_sensors()
        self.gravity_mag = float(abs(self.model.opt.gravity[2]))

    def camera(self, camera_name: str) -> Camera:
        return Camera(camera_name=camera_name, parent_agent=self)
    ...
    def _load_model(self) -> None:
        ...
        Note:
            ...
            :class:`safety_gymnasium_drones.World` via :meth:`set_engine`.
        """
        ...
        self.set_engine(Engine(self.model, self.data))
    ...
    def _build_action_space(self) -> gymnasium.spaces.Dict:
        ...
        for i in range(self.num):
            act_space[f'agent_{i}'] = single_space
        return act_space
    ...
    def build_sensor_observation_space(self) -> gymnasium.spaces.Dict:
        ...
        return gymnasium.spaces.Dict(obs_space_dict)

    def build_observation_space(self) -> gymnasium.spaces.Dict:
        """Build observation space for all agent.
        ...
        for index in range(self.num):
            sensor_obs_space = self.build_sensor_observation_space()
            obs_space_dict[f'observation__{index}'] = sensor_obs_space

        return gymnasium.spaces.Dict(obs_space_dict)

    def obs_sensor(self) -> dict[str, dict[str, np.ndarray]]:
        ...
        for index in range(self.num):
            ...
            for sensor in self.sensor_conf.sensors:  # Explicitly listed sensors
                single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
            for sensor in self.sensor_info.hinge_vel_names:
                single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
            for sensor in self.sensor_info.ballangvel_names:
                single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
            if self.sensor_info.freejoint_pos_name:
                sensor = self.sensor_info.freejoint_pos_name
                single_obs[sensor] = self.get_sensor(sensor + f'__{index}')[2:]
            if self.sensor_info.freejoint_qvel_name:
                sensor = self.sensor_info.freejoint_qvel_name
                single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
            ...
            if self.sensor_conf.sensors_angle_components:
                for sensor in self.sensor_info.hinge_pos_names:
                    ...
                    single_obs[sensor] = np.array([np.sin(theta), np.cos(theta)])
                for sensor in self.sensor_info.ballquat_names:
                    quat = self.get_sensor(sensor + f'__{index}')
                    single_obs[sensor] = quat2mat(quat)
            else:  # Otherwise read sensors directly
                for sensor in self.sensor_info.hinge_pos_names:
                    single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
                for sensor in self.sensor_info.ballquat_names:
                    single_obs[sensor] = self.get_sensor(sensor + f'__{index}')
            obs[f'observation__{index}'] = single_obs
        return obs
    ...
    def process_config(self, config: dict, layout: dict, rots: float) -> None:
        ...
        Note:
            This method is called in :meth:`safety_gymnasium_drones.bases.base_task._build_world_config` to
            fill the configuration dictionary which used to generate mujoco instance xml string of
            environments in :meth:`safety_gymnasium_drones.World.build`.
        """
        assert (
            len(rots) == self.num
    ...
    def dist_xyz(self, pos: np.ndarray, index: int) -> float:
    ...
    def world_xyz(self, pos: np.ndarray) -> np.ndarray:
        ...
        """
        assert pos.shape == (3,)
        return pos - self.agents.agent_pos()  # pylint: disable=no-member
    ...
    def quat(self, index) -> np.ndarray:
        """Get the position of the agent in the simulator world reference frame.

        Returns:
            np.ndarray: The Cartesian position of the agent.
        """
        return self.engine.data.body(f'agent__{index}').xquat.copy()

    def euler(self, index) -> np.ndarray:
        """Get the position of the agent in the simulator world reference frame.

        Returns:
            np.ndarray: The euler angle of the agent.
        """
        return quat2euler(self.engine.data.body(f'agent__{index}').xquat.copy())
