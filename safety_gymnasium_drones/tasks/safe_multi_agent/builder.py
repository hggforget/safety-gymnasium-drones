...
from safety_gymnasium_drones.tasks.safe_multi_agent.bases.base_task import BaseTask
...
class Builder(gymnasium.Env, gymnasium.utils.EzPickle):
    ...
    def reset(
        ...
        *,
        ...
        #     # state_agent = dict(filter(lambda x: x[0][-1] == agents[0][-1], state.items()))
    ...
    def step(self, action: dict) -> tuple[np.ndarray, float, float, bool, bool, dict]:
        ...
        if not self.task.agents.is_all_alive():
            self.terminated = True
        ...
        return state, sum(rewards.values()), sum(costs.values()), self.terminated, self.truncated, info
    ...
    @property
    def action_dict(self) -> gymnasium.spaces.Dict:
        """Helper to get action space."""
        return gymnasium.spaces.Dict(self.task.action_space)
