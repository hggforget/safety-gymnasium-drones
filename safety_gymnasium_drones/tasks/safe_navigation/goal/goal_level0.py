...
from safety_gymnasium_drones.assets.geoms import Goal
from safety_gymnasium_drones.bases.base_task import BaseTask
import numpy as np
from safety_gymnasium_drones.utils.task_utils import quat2vel, angle_between_vectors, quat2euler
...
class GoalLevel0(BaseTask):
    ...
    def calculate_reward(self, **kwargs):
        ...
        # print(f"_perception_reward: {self._perception_reward * math.exp(self._perception_reward_lambda * math.pow(self.optical_axis_angle[0], 4))}")
        # # euler = self.agent.euler / np.pi
        # reward -= self._euler_penalty * np.sqrt(np.sum(np.square(self.agent.euler[:2])))
        # print(f"_euler_penalty: {- self._euler_penalty * np.sqrt(np.sum(np.square(self.agent.euler[:2])))}")
        reward -= self._angular_vel_penalty * np.sqrt(np.sum(np.square(self.agent.angular_vel)))
        # print(f"_angular_vel_penalty: {- self._angular_vel_penalty * np.sqrt(np.sum(np.square(self.agent.angular_vel)))}" )
        dist_goal = self.dist_goal()
        reward += (self.last_dist_goal - dist_goal) * self.goal.reward_distance
        # print(
        #     f"reward_distance: {(self.last_dist_goal - dist_goal) * self.goal.reward_distance}")
